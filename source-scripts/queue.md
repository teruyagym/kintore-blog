# 台本転用キュー

週2回の自動投稿ジョブが、ここから未使用の台本を1つ選んでブログ記事に変換する。
すでに公開済みのブログ記事(`content/posts/`)とテーマが重複しないよう、必ず既存記事のタイトル・keywordを確認してから選ぶこと。

**重要：台本をそのままコピーしない。** 話し言葉（「ではここから見ていきます」等のナレーション表現）を削除し、SEOキーワードを意識した見出し構成に再構成し、written article として自然な文体に書き直すこと。既存の`content/posts/`内の記事を文体・構成の参考にする。

## 未使用

- episode63-protein-2g-muda.md（プロテイン体重×2gは無駄、摂取フェーズ）
- episode74-kinpidai-kanzen-kouryaku.md（筋肥大完全攻略、総合）
- episode88-onaka-dake-ochinai-7sen.md（お腹だけ落ちない7選、代謝フェーズ）
- episode68-zettai-7sen.md（絶対NG7選、総合）
- episode80-tsukanai-hito-7sen.md（筋肉がつかない人の共通点7選、総合）
- episode87-kinniku-tsukanai-ng-training-7sen.md（筋肉がつかないNGトレーニング7選、刺激フェーズ）
- episode95-seichou-hayai-hito-8sen.md（成長が早い人の特徴8選、総合）
- episode100-seichou-5tsu-no-shikumi.md（筋肉が成長する5つの仕組み、総合）
- episode101-shinjoushiki-5sen.md（筋トレの新常識5選、総合）

## 使用済み

- 2026-07-23-episode89-1nen-tsuzuketemo-kawaranai-7sen.md（1年続けても変わらない7選、総合）→ 2026-09-17公開: content/posts/kintore-tsuzuketemo-kawaranai-riyuu.md

- 2026-07-28-episode92-mainichi-kintore-yamete.md（毎日筋トレをやめるべき理由、回復フェーズ）→ 2026-09-14公開: content/posts/mainichi-kintore-kinniku-fuenai.md

- 2026-06-27-episode78-kinniku-tsuku-hito-shokuji-10sen.md（筋肉がつく人の食事10選、摂取フェーズ）→ 2026-09-10公開: content/posts/kinniku-eiyoso-migaotoshi.md

- 2026-06-04-episode69-3months-7sen.md（3ヶ月続けても変わらない7選、総合）→ 2026-09-07公開: content/posts/kinniku-switch-hairanai-riyuu.md
- 2026-05-24-episode64-tsuku-tsukanai-7differences.md（筋肉がつく人つかない人の違い7選、総合）→ 2026-09-03公開: content/posts/kintore-seika-deru-hito-denai-hito.md
- 2026-07-30-episode93-koka-nibai-3gijutsu.md（効果が2倍になる3つの技術、刺激フェーズ）→ 2026-08-31公開: content/posts/kintore-koka-sa-shigeki-no-shitsu.md
- 2026-05-18-episode61-oidoikonde-kawaranai.md（追い込んでも変わらない、回復フェーズ）→ 2026-08-27公開: content/posts/oikomisugi-kinnikutsuu-kinniku-fuenai.md
- 2026-05-30-episode67-yameru-7sen.md（筋肉をつけたいなら今すぐやめること7選、総合）→ 2026-08-24公開: content/posts/kintore-kouka-kesu-seikatsu-shukan.md
- 2026-05-25-episode65-uso7.md（筋トレの嘘7選、総合）→ 2026-08-20公開: content/posts/kintore-omoikomi-40dai.md
- 2026-06-08-episode70-bulkup-10sen.md（バルクアップ10選、摂取フェーズ）→ 2026-08-17公開: content/posts/tabete-mo-taiju-fuenai-riyuu.md
- 2026-06-28-episode79-manseiensho.md（慢性炎症、代謝フェーズ）→ 2026-08-10公開: content/posts/manseiensho-kinniku-fuenai.md
- 2026-06-16-episode73-kinniku-sign-7.md（筋肉がつく前兆サイン7選、回復フェーズ）→ 2026-08-10公開: content/posts/kinniku-tsuitekita-sign.md
- 2026-07-07-episode82-cho-naikankyou-7sen.md（腸内環境7選、消化吸収フェーズ）→ 2026-08-13公開: content/posts/cho-naikankyou-kinniku-tsukanai.md

## 除外方針

- 痛み・ケガ系のテーマは対象外（`content/keyword-queue.md`と同じ理由）
- ダイエット・脂肪系のみを主語にしたテーマは避け、筋肉系を主語にする（`project_youtube_diet_vs_muscle_theme`の知見と同じ）
- すでに`content/posts/`にある記事とテーマが近い場合はスキップし、次の候補を使う
